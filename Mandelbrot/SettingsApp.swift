...
private final class SettingsAppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        let window = controller.configureSheet()
        window.center()
...
private enum SettingsApp {
    ...
    private static func runSmokeTest() {
        ...
        precondition(window.title == "Mandelbrot Screensaver Options")
        precondition(sliders.count == 1, "Expected one zoom slider")
        precondition(popups.count == 3, "Expected three option menus")
        precondition(buttons.count >= 5, "Expected checkboxes and action buttons")
